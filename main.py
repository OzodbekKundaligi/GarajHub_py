...
def show_startup_page(chat_id, page):
    # Pagination storage - to track current position
    if not hasattr(show_startup_page, 'current_page'):
        show_startup_page.current_page = {}
    show_startup_page.current_page[chat_id] = page
    
    startups, total = get_active_startups(page, per_page=1)  # 1 per page for better experience
...
def handle_join_startup(call):
    try:
        ...
        if request_id:
            ...
            # Send notification to startup owner - GUARANTEED
            ...
            if startup and user:
                ...
                try:
                    # Send to owner with photo if available
                    if user.get('photo_id'):  # If stored
                    ...
                    bot.send_message(call.message.chat.id, f"⚠️ Egaga xabar yuborilishda muammo. Direktno @{startup['owner_id']} ga yozing.", show_alert=False)
            else:
...
def admin_approve_startup(call):
    ...
    try:
        ...
        try:
            ...
            markup = InlineKeyboardMarkup()
            # Use callback_data so pressing the button creates a join request
            markup.add(InlineKeyboardButton('🤝 Startupga qo\'shilish', 
                                           callback_data=f'join_startup_{startup_id}'))
...
def handle_back_button(message):
    ...
    if user_state == 'in_profile' or user_state.startswith('editing_'):
        ...
        # Agar state aniqlanmagan bo'lsa, asosiy menyuga qayt
        clear_user_state(user_id)
        show_main_menu(message)
...
def handle_other_messages(message):
    if message.chat.id == ADMIN_ID and message.text != '🔙 Orqaga' and message.text != '🛠 Admin panel':
        # Admin panelda boshqa tugmalar bosilganda
        admin_panel(message)
...
if __name__ == '__main__':
    ...
    try:
        # Remove any existing webhook (prevents '409 Conflict' when another getUpdates/webhook is active)
        try:
            bot.remove_webhook()
        except Exception:
            pass
