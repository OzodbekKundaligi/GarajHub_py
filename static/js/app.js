...
function showAdminPanel() {
	...
	document
		.querySelector('.menu li[data-page="dashboard"]')
...
async function loadStatistics() {
	try {
		...
		if (response.ok) {
			...
			document.getElementById('activeStartups').textContent =
				stats.active_startups.toLocaleString()
			document.getElementById('newToday').textContent =
				stats.new_users_today + stats.new_startups_today
			...
			document.getElementById(
				'activeStartupCount'
			).textContent = `${stats.active_startups} ta`
			document.getElementById(
				'newUsersToday'
			).textContent = `${stats.new_users_today} user`

			// Faollik darajasi
			const activityRate = stats.user_growth_rate
			document.getElementById('activityRate').textContent = `${activityRate}%`
			document.getElementById('activityProgress').style.width = `${Math.min(
				activityRate,
				100
			)}%`

			// Chart markazidagi raqam
			document.getElementById('totalStartupsChart').textContent =
...
async function loadUserGrowthChart() {
	try {
		...
		const response = await fetch(
			`${API_BASE_URL}/statistics/chart/user_growth?period=${period}`,
			{
				headers: {
					Authorization: `Bearer ${currentToken}`,
				},
...
async function loadStartupDistributionChart() {
	try {
		const response = await fetch(
			`${API_BASE_URL}/statistics/chart/startup_distribution`,
			{
				headers: {
					Authorization: `Bearer ${currentToken}`,
				},
		...
		if (response.ok) {
			...
			const ctx = document
				.getElementById('startupDistributionChart')
				.getContext('2d')
...
async function loadActivityChart() {
	try {
		...
		const response = await fetch(
			`${API_BASE_URL}/statistics/chart/activity?period=${period}`,
			{
				headers: {
					Authorization: `Bearer ${currentToken}`,
				},
...
function updateDistributionLegend(stats) {
	...
	const statuses = [
		...
		{
			label: 'Kutilayotgan',
			value: stats.pending_startups,
			color: '#FF9F40',
			type: 'pending',
		},
		{
			label: 'Yakunlangan',
			value: stats.completed_startups,
			color: '#6DC5A3',
			type: 'completed',
		},
		{
			label: 'Rad etilgan',
			value: stats.rejected_startups,
			color: '#E74C3C',
			type: 'rejected',
		},
	]
	...
	legend.innerHTML =
		html || '<div class="empty-state"><p>Ma\'lumot yo\'q</p></div>'
}
			...
			if (data.users.length === 0) {
				...
				data.users.forEach(user => {
					...
					row.innerHTML = `
                        ...
                        <td>
                            <button class="action-btn view-btn" onclick="viewUser(${
															user.user_id
														})" title="Ko'rish">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button class="action-btn edit-btn" onclick="editUserStatus(${
															user.user_id
														})" title="Tahrirlash">
			...
			if (data.startups.length === 0) {
				...
				data.startups.forEach(startup => {
					...
					row.innerHTML = `
                        ...
                        <td>${startup.name}</td>
                        <td>${startup.first_name} ${
						startup.last_name || ''
					}</td>
                        <td><span class="status-badge ${startup.status}">${
						startup.status
					}</span></td>
                        <td>${formatDate(startup.created_at)}</td>
                        <td>${startup.member_count}</td>
                        <td>
                            <button class="action-btn view-btn" onclick="viewStartup(${
															startup.startup_id
														})" title="Ko'rish">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button class="action-btn edit-btn" onclick="editStartup(${
															startup.startup_id
														})" title="Tahrirlash">
                                <i class="fas fa-edit"></i>
                            </button>
...
async function loadDetailedStatistics() {
	try {
		...
		if (statsResponse.ok) {
			...
			document.getElementById('detailedTotalUsers').textContent =
				stats.total_users.toLocaleString()
			document.getElementById('detailedActiveUsers').textContent =
				stats.total_users.toLocaleString() // Assuming all are active
			document.getElementById('detailedNewUsers').textContent =
				stats.new_users_today.toLocaleString()
			document.getElementById('detailedAvgDailyUsers').textContent = Math.round(
				stats.users_last_month / 30
			).toLocaleString()

			// Startap statistikasi
			document.getElementById('detailedTotalStartups').textContent =
				stats.total_startups.toLocaleString()
			document.getElementById('detailedActiveStartups').textContent =
				stats.active_startups.toLocaleString()
			document.getElementById('detailedNewStartups').textContent =
				stats.new_startups_today.toLocaleString()

			const successRate =
				stats.total_startups > 0
					? Math.round((stats.completed_startups / stats.total_startups) * 100)
					: 0
			document.getElementById(
				'detailedSuccessRate'
			...
			if (data.admins.length === 0) {
				...
				data.admins.forEach(admin => {
					...
					row.innerHTML = `
                        ...
                        <td>${admin.email}</td>
                        <td><span class="role-badge ${admin.role}">${
						admin.role
					}</span></td>
                        <td>${
													admin.last_login
                    ...
                    <div class="user-header">
                        <div class="user-avatar-large">
                            <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(
														...
														)}&background=4A6FA5&color=fff" alt="User">
                        </div>
                        <div class="user-info-large">
                            <h3>${data.user.first_name} ${
				data.user.last_name || ''
			}</h3>
			...
			modalContent.innerHTML = `
                <div class="startup-detail">
                    <div class="startup-header">
                        ...
                        <span class="status-badge ${data.startup.status}">${
				data.startup.status
			}</span>
                    </div>
                    ...
                    <div class="startup-meta">
                        <div class="meta-item">
                            ...
                            <span>Muallif: ${data.startup.first_name} ${
				data.startup.last_name || ''
			}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-calendar"></i>
                            <span>Yaratilgan: ${formatDate(
															data.startup.created_at
														)}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-link"></i>
                            <span>Guruh: <a href="${
															data.startup.group_link
														}" target="_blank">${
				data.startup.group_link
			}</a></span>
                        </div>
                    ...
                    <div class="startup-description">
                        <h4>Tavsif:</h4>
                        <p>${
													data.startup.description || 'Tavsif mavjud emas'
												}</p>
                    </div>
...
async function editStartup(startupId) {
	const newStatus = prompt(
		'Yangi holatni tanlang (pending, active, completed, rejected):'
	)
	if (
		!newStatus ||
		!['pending', 'active', 'completed', 'rejected'].includes(newStatus)
	) {
		showToast("Noto'g'ri holat", 'warning')
		return
	}
...
function openFullscreenChart(chartId, title) {
	...
	const chart =
		chartId === 'userGrowthChart'
			? userGrowthChart
			: chartId === 'startupDistributionChart'
...
function updatePagination(elementId, currentPage, totalPages, callback) {
	...
	// Oldingi tugma
	if (currentPage > 1) {
		html += `<button class="pagination-btn" onclick="${callback.name}(${
			currentPage - 1
